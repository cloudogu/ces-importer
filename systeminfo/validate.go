...
type systemInfoProvider interface {
	...
	getExporterSystemInfo(conf configuration.Configuration) (*systemInfo, error)
...
func NewValidator(conf configuration.Configuration, ctx context.Context, namespace string, p systemInfoProvider) *Validator {
	return &Validator{
		conf:               conf,
		ctx:                ctx,
		namespace:          namespace,
...
func (v *Validator) ValidateSystemInfo() error {
	...
	imSystemInfo, err := v.systemInfoProvider.getSystemInfo()
	if err != nil {
		return fmt.Errorf("could not get importer system info: %s", err)
	}
	exSystemInfo, err := v.systemInfoProvider.getExporterSystemInfo(v.conf)
	if err != nil {
	...
	for _, exDogu := range exInfo.Dogus {
		...
		if imDogu.Name == "" {
			...
			if !(imDogu.Version == exDogu.Version) {
				...
				result = v.updatePVC(exDogu, imDogu, provider, result)
			}
		}
	}
	// validate components
...
func (v *Validator) updatePVC(exDogu dogu, imDogu dogu, pvcs pvcClient, result *multierror.Error) *multierror.Error {
